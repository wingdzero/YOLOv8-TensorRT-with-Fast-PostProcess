...
class Mosaic(BaseMixTransform):
    """
    这个类通过将多个（4个或9个）图像组合成一个单一的马赛克图像来进行马赛克数据增强。该数据增强以给定的概率应用于数据集

    属性：
        dataset：应用马赛克数据增强的数据集
        imgsz (int, optional)：马赛克处理后单个图像的尺寸（高度和宽度）。默认为 640
        p (float, optional)：应用马赛克数据增强的概率。必须在0-1的范围内。默认为 1.0
        n (int, optional)：网格大小，可以是4（表示2x2）或9（表示3x3）
    """
...
class RandomPerspective:
    """
    实现了对图像及其对应的边界框、分割区域和关键点进行随机透视和仿射变换。这些变换包括旋转、平移、缩放和剪切。此类还提供了一个选项，可以以指定的概率有条件地应用这些变换

    属性：
        degrees (float)：随机旋转的角度范围
        translate (float)：随机平移的宽度和高度的比例
        scale (float)：缩放因子区间，例如，缩放因子为 0.1 时允许在 90%-110% 之间调整大小
        shear (float)：剪切强度（以度为单位）
        perspective (float)：透视畸变因子
        border (tuple)：指定马赛克边界的元组
        pre_transform (callable)：在开始随机变换之前应用于图像的函数/变换
    方法：
        affine_transform(img, border)：对图像应用一系列仿射变换
        apply_bboxes(bboxes, M)：使用计算出的仿射矩阵变换边界框
        apply_segments(segments, M)：变换分割区域并生成新的边界框
        apply_keypoints(keypoints, M)：变换关键点
        call(labels)：主方法，对图像及其对应的注释应用变换
        box_candidates(box1, box2)：筛选出在变换后不符合特定标准的边界框
    """
    ...
    def __init__(
        ...
        self.degrees = degrees  #旋转
        self.translate = translate  # 比例调整
        self.scale = scale  # 缩放
        self.shear = shear  # 剪切
        self.perspective = perspective  # 透视畸变
        self.border = border  # mosaic 边界
        self.pre_transform = pre_transform  # 在开始随机变换之前应用于图像的函数/变换
...
class CopyPaste:
    """
    实现了论文 https://arxiv.org/abs/2012.07177 中描述的复制粘贴数据增强方法。此类负责对图像及其对应的实例应用复制粘贴数据增强
    """
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    pre_transform = Compose(
        [
            ...
            CopyPaste(p=hyp.copy_paste),  # 复制粘贴数据增强
            RandomPerspective(
                degrees=hyp.degrees,
    ...
    flip_idx = dataset.data.get("flip_idx", [])  # 关键点数据增强, 左右翻转，跟随图像的左右翻转概率(fliplr)
