...
def yaml_load(file="data.yaml", append_filename=False):
    ...
    """
    assert Path(file).suffix in {".yaml", ".yml"}, f"Attempting to load non-YAML file {file} with yaml_load()"  # 首先确保读取的时yaml文件
    with open(file, errors="ignore", encoding="utf-8") as f:
        s = f.read()  # string

        # 使用正则表达式移除特殊字符(\x09:制表符、\x0A:换行符、\x0D:回车符、\x20-\x7E:ASCII 可打印字符范围、\x85:下一行符、\xA0-\uD7FF:Unicode 可打印字符范围的一部分、\uE000-\uFFFD:Unicode 可打印字符范围的另一部分、\U00010000-\U0010ffff:补充平面的可打印字符范围)
        if not s.isprintable():
            s = re.sub(r"[^\x09\x0A\x0D\x20-\x7E\x85\xA0-\uD7FF\uE000-\uFFFD\U00010000-\U0010ffff]+", "", s)  # re.sub() 将所有不匹配这些范围的字符替换为空字符串，从而删除不可打印字符

        # Add YAML filename to dict and return
        data = yaml.safe_load(s) or {}  # 使用yaml读取数据集yaml内容，返回一个字典 (yaml.safe_load() may return None for empty files)
        if append_filename:
            data["yaml_file"] = str(file)  # 将数据集yaml文件路径也添加到数据集信息(data)中
        return data
