...
def check_imgsz(imgsz, stride=32, min_dim=1, max_dim=2, floor=0):
    """
    要验证图像尺寸是否为给定步幅（stride）的倍数，并在图像尺寸不是步幅倍数时将其更新为大于或等于给定下限（floor value）的最近步幅倍数
    ...
    # Apply max_dim
    if len(imgsz) > max_dim:  # 检查输入分辨率是否是一个数
        msg = (
...
def check_suffix(file="yolov8n.pt", suffix=".pt", msg=""):
    ...
    if file and suffix:
        ...
        for f in file if isinstance(file, (list, tuple)) else [file]:
            s = Path(f).suffix.lower().strip()  # 获取文件扩展名。其中.suffix获取扩展名，.lower()将扩展名转换成小写，.strip()去除两端空格
            if len(s):
...
def check_model_file_from_stem(model="yolov8n"):
    ...
    if model and not Path(model).suffix and Path(model).stem in downloads.GITHUB_ASSETS_STEMS:  # .suffix返回文件扩展名，.stem返回文件名
        return Path(model).with_suffix(".pt")  # 如果没有扩展名，自动添加, i.e. yolov8n -> yolov8n.pt
    else:
        return model
...
def check_file(file, suffix="", download=True, hard=True):
    ...
    file = str(file).strip()  # 确保yaml文件没有多余的空格
...
def check_amp(model):
    """
    该函数检查 YOLOv8 模型的 PyTorch 自动混合精度（AMP）功能。如果检查失败，意味着系统上的 AMP 存在异常，可能会导致 NaN 损失或零 mAP 结果，因此在训练期间将禁用 AMP。
    参数:
        model (nn.Module): YOLOv8 模型实例。
    示例:
        ...
        check_amp(model)
    返回:
        (bool): 如果 AMP 功能在 YOLOv8 模型上正常工作，则返回 True，否则返回 False。
    """
